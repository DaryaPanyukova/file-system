...
public class FileCopyParser : ICommandParser
{
    ...
    public IArgumentParser<FileCopyCommandBuilder>? ArgumentParser { get; set; }
    ...
    public Result Parse(IStringIterator input)
    {
        ...
        if (word == "copy")
        {
            result = ArgumentParser == null
                ? new Result(new Status.Success(), new FileCopyCommand(null, null))
                : ArgumentParser.Parse(input, new FileCopyCommandBuilder());
        }
        else
        {
            input.PutTerm();
